        ...
        @if ($usuconsulta)
            <div class="ring-1 ring-zinc-600 rounded-md m-6  p-2">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    Diligencias asignadas a: <span class=" uppercase font-bold">{{$usuconsulta->name}}</span>, se eligen las diligencias para la ciudad de: <span class="uppercase font-bold">{{$ubicacion->sucursal->ciudad->name}}</span>
                </div>
            </div>
        @endif
                                                ...
                                                @can('di_diligestmensa')
                                                    ...
                                                    @foreach ($item->mensajeros as $val)
                                                        @if ($usuconsulta->id===$val->user_id && $val->status===2)
                                                            <button wire:click.prevent="asignar({{$item->id}})" type="button" class="inline-flex items-center p-1 text-xs font-medium text-green-900 bg-gradient-to-r from-green-300 via-green-400 to-green-500 border border-green-900 rounded-lg hover:bg-green-900 hover:text-white focus:z-10 focus:ring-2 focus:ring-green-500 focus:bg-green-900 focus:text-white dark:border-white dark:text-white dark:hover:text-white dark:hover:bg-green-700 dark:focus:bg-green-700">
